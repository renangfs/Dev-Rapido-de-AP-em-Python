...
def GravaDados(vnome,vmatri,vn1,vn2):#função para gravar dados
    ...
    try:#Tratamento de erro
        dados = vmatri+',\''+vnome+'\',\''+vn1+'\',\''+vn2+'\',\''+media+'\')'#padrões (\'') para por aspa no inicio ('\')para por aspa no final
        #print (dados) ...ver as ordens dos dados
        comando="""INSERT INTO aluno (matricula,nome,nota1,nota2,media) VALUES("""           
        sql = comando+dados   
...
def consultar():
    ...
    try:
        ...
        if linhas == linhas:
                voltar = Button(app, text="voltar",bg="#808080",fg="#010101",font="consolas 25 bold",command=lambda: menu()).place(x=10,y=20,width=130,height=50)# Cria o botão e envia para função Gravar dados
                voltar.pack() 
